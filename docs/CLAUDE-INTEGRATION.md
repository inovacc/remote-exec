...
| Path | What it is |
|------|-----------|
| `.claude/commands/remote/enroll.md` | `/remote:enroll <endpoint> --token <t>` — join an agent, pin its identity |
...
1. On the agent host: `rexec-agentd ca init` then `rexec-agentd service install && rexec-agentd service start` (or run it directly).
2. Operator issues a scoped, single-use token: `rexec-agentd token new --role rex:operator`.
3. Claude enrolls: `/remote:enroll <host:port> --token <t>` → the agent signs a client cert,
...
what the controller may call — a reader cert cannot invoke `run`, and only an admin cert can
reach `deploy`.
...
rexec deploy <cmd>
  ...
  └─ agent policy = ask   → APPROVAL_REQUIRED approval_id=… operation=… reason=…
                             ...
                             ├─ Approve → rexec deploy --approval <id> <cmd>  (runs, single-use)
...
Each subagent confirms its target via `rexec id`, does its work, handles any approval prompt,
and returns a compact result. Siblings never touch each other's credentials.
