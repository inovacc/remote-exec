...
- [x] Per-instance identity + fingerprint pinning (`rexec id`)
- [x] Role-based authorization from the client cert (`rex:reader ⊂ operator ⊂ admin`)
- [x] Streaming remote execution (`rexec run`) — build/test/analyze
- [x] Destructive-op gate (`rexec deploy`): policy + single-use live approval
...
- [ ] Optional `--expect-fingerprint` on `rexec enroll` (close the bootstrap MITM window)
